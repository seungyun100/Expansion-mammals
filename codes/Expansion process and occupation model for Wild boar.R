...
#####################################################################
##### Estimates the range expansion process of wild boar        #####
...
df_t1 <- "./data/animal/wildboar/wildboar_1978.shp"
df_t2 <- "./data/animal/wildboar/wildboar_2003_new.shp"
...
wildboar_p1p2_fit <- clogit(population ~ scale(AG) + scale(adjacent_counts) + scale(TRImean) + scale(aband) + scale(snow) +
                             scale(NL) + strata(source_mesh_code), data = adjacent_grids_df_, model = TRUE)

#vif(wildboar_p1p2_fit)
summary(wildboar_p1p2_fit)
...
#####################################################################
##### Estimates the range expansion process of wild boar        #####
##### and analyzes the drivers of its expansion (2003-2014)     #####
#####################################################################


species <- "wildboar"
covariables <- enviro2014


########### Step 1. Data Preparation

### Setting the path for SHP files and reading the data
df_t1 <- "./data/animal/wildboar/wildboar_2003.shp"
df_t2 <- "./data/animal/wildboar/wildboar_2014_new.shp"
grids <- st_read("./data/grid/mesh5.shp")
...
wildboar_p2p3_fit <- clogit(population ~ scale(AG) + scale(adjacent_counts) + scale(TRImean) + scale(aband) + scale(snow) +
                             scale(NL) + strata(source_mesh_code), data = adjacent_grids_df_, model = TRUE)

#vif(wildboar_p2p3_fit)
summary(wildboar_p2p3_fit)
